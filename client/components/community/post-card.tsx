...
export function PostCard({ post }: { post: CommunityPost }) {
  ...
  const categoryMap = {
    "Anxiety": "category_anxiety",
    "Depression": "category_depression", 
    "Mindfulness": "category_mindfulness",
    "Sleep": "category_sleep",
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="text-xs md:text-sm font-medium text-slate-100 truncate">
                  {post.user.isAnonymous ? t("anonymous") : t(post.user.name)}
                </span>
                <span className="rounded-full border border-white/15 px-1.5 py-0.5 text-[9px] md:text-[10px] font-medium text-slate-300 flex-shrink-0">
                  {t(categoryMap[post.category] || post.category)}
                </span>
              </div>
              <span className="text-[10px] md:text-xs text-slate-500">{timeSince(post.createdAt)} {t("time_ago")}</span>
            </div>
          </div>
        </div>
      </header>
