...
const INITIAL_RESOURCES: Resource[] = [
  {
    id: "r1",
    title: "breathing_techniques_anxiety",
    description: "breathing_techniques_desc",
  ...
  {
    id: "r2",
    title: "cognitive_restructuring_worksheet",
    description: "cognitive_restructuring_desc",
    category: "Depression",
    format: "Worksheet",
    popularity: 87,
  ...
  {
    id: "r3",
    title: "sleep_hygiene_guide",
    description: "sleep_hygiene_desc",
    category: "Sleep",
    format: "Article",
    popularity: 76,
  ...
  {
    id: "r4",
    title: "body_scan_meditation",
    description: "body_scan_desc",
    category: "Mindfulness",
    format: "Video",
    popularity: 65,
  ...
  {
    id: "r5",
    title: "grounding_techniques",
    description: "grounding_techniques_desc",
    category: "Anxiety",
    format: "Guide",
    popularity: 92,
  ...
  {
    id: "r6",
    title: "focus_productivity_tips",
    description: "focus_productivity_desc",
...
const CATEGORY_TRANSLATION_MAP: Record<CategoryFilter, string> = {
  "All": "all",
  "Anxiety": "anxiety",
...
const FORMAT_TRANSLATION_MAP: Record<FormatFilter, string> = {
  "All": "all",
  "Article": "article",
  "Video": "video", 
  "Worksheet": "worksheet",
...
const SORT_TRANSLATION_MAP: Record<SortOption, string> = {
...
export default function ResourcesPage() {
  ...
  const filtered = useMemo(() => {
    let list = INITIAL_RESOURCES.filter((r) => {
      ...
      const matchesSearch =
        !debounced || 
        t(r.title).toLowerCase().includes(debounced) || 
        t(r.description).toLowerCase().includes(debounced) ||
    ...
    return list
  }, [category, format, debounced, sort, t])
      ...
      <main className="overflow-y-auto hide-scrollbar text-slate-100 relative" style={{height: '100vh'}}>
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder={t("search_resources_placeholder")}
                className={cn(
        ...
        <section className="mt-6 sm:mt-8 lg:mt-10 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
              ...
              {filtered.length > 0 ? (
                  ...
                  {filtered.map((r) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Button
                          aria-label={savedIds.includes(r.id) ? t("unsave") : t("save")}
                          ...
                          title={savedIds.includes(r.id) ? t("saved") : t("save")}
                        >
                          <svg
                            width="18"
                            height="18"
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <span className="inline-flex items-center rounded-full bg-white/5 border border-white/10 px-2.5 py-1 text-xs text-slate-100">
                          {t(CATEGORY_TRANSLATION_MAP[r.category as CategoryFilter] || r.category.toLowerCase())}
                        </span>
                ...
                >
                  <img
                    ...
                    width={120}
                    height={120}
                    alt={t("no_results_illustration")}
                    className="mx-auto opacity-80"
                  />
                  <h3 className="mt-6 text-lg font-medium text-slate-100">{t("no_results_found_resources")}</h3>
                  <p className="mt-1 text-slate-300">{t("try_adjusting_search_filters")}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <aside className="lg:col-span-3">
            ...
            >
              <SectionBlock title={t("featured_guides_title")}>
              ...
              <SectionBlock title={t("recently_viewed_title")}>
                {recent.length === 0 ? (
                  <p className="text-sm text-slate-300">{t("nothing_yet_text")}</p>
                ) : (
              ...
              <SectionBlock title={t("saved_resources_title")}>
                {saved.length === 0 ? (
                  <p className="text-sm text-slate-300">{t("nothing_saved_yet")}</p>
                ) : (
                  <ul className="space-y-2">
                    {saved.map((r) => (
                      <li key={r.id} className="flex items-center justify-between gap-2">
