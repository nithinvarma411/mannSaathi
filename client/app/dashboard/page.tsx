  ...
  <div className="rounded-xl sm:rounded-2xl border border-white/20  bg-black p-3 sm:p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">{t("recent_booking_activity")}</p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between gap-2 sm:gap-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-slate-500">
                  {b.counsellor} • {t(`booking_time_${b.time.toLowerCase().replace(/ /g, '_').replace(':', '').replace('.', '').replace(',', '')}`)}
                </div>
              </div>
              <span className="shrink-0 rounded-full border border-white/20 bg-white/5 px-2 py-0.5 text-xs text-slate-300">
                {t(`booking_status_${b.status.toLowerCase()}`)}
              </span>
            </div>
...
function ChatsHistoryCard() {
  const { t } = useLanguage()
  const threads = [
    { id: "TH-5921", student: "Anon-7F3A", last: "chat_panic_feelings_improved", when: "09:41" },
    { id: "TH-5918", student: "Anon-2C9D", last: "chat_discussed_sleep_hygiene", when: "08:22" },
    { id: "TH-5907", student: "Anon-8B12", last: "chat_explored_motivation_triggers", when: "yesterday" },
  ] as const
  ...
  <div className="rounded-xl sm:rounded-2xl border border-white/20 p-3 sm:p-4 bg-black">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">{t("recent_conversations")}</p>
        </div>
      </div>
      <ul className="mt-3 space-y-2 sm:space-y-3">
        {threads.map((thread) => (
          <li key={thread.id} className="rounded-lg sm:rounded-xl border border-white/20 bg-black p-2 sm:p-3">
            <div className="flex items-center justify-between gap-2 sm:gap-3">
              ...
              <div className="shrink-0 text-xs text-slate-500">{t(thread.when)}</div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 sm:gap-4">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <span className="mr-2">{t("select_timeframe")}: {t(`timeframe_${timeframe.toLowerCase().replace(/ /g, '_')}`)}</span>
                ...
                <DropdownMenuContent align="end" className="min-w-56 border-white/10 bg-slate-900 text-slate-100">
                  ...
                  {(["Today", "Last 7 Days", "Last 30 Days", "Custom"] as Timeframe[]).map((tf) => (
                    <DropdownMenuItem key={tf} onClick={() => setTimeframe(tf)} className="cursor-pointer">
                      {t(`timeframe_${tf.toLowerCase().replace(/ /g, '_')}`)}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <div className="rounded-xl sm:rounded-2xl border border-white/20 bg-black p-3 sm:p-4 transition-all hover:bg-white/7 hover:-translate-y-0.5 cursor-pointer">
                <div className="flex items-center">
                  ...
                  <div className="ml-3 sm:ml-4">
                    ...
                    <p className="text-xs sm:text-sm text-slate-500">{t("view_institutional_analytics")}</p>
                  </div>
                </div>
              </div>
            </Link>
