...
const CATEGORIES = ["All", "Anxiety", "Depression", "Mindfulness", "Sleep", "Relationships"] as const
...
const INITIAL_POSTS: CommunityPost[] = [
  {
    ...
    content: "Feeling overwhelmed before presentations. Any tips that worked for you to calm nerves quickly?",
    likes: 12,
    comments: 5,
    shares: 1,
  },
  {
    id: "p2",
    user: { name: "Maya Singh", avatarUrl: "/diverse-user-avatars.png", isAnonymous: false },
    category: "Mindfulness",
    createdAt: new Date(Date.now() - 1000 * 60 * 60 * 4).toISOString(), // 4h
    content: "A 5-minute breathing practice (4-7-8) helped me sleep better. Sharing in case it helps someone else too.",
    likes: 34,
    comments: 9,
    shares: 4,
  },
  {
    ...
    content: "I keep waking up around 3am and struggle to fall asleep again. What has helped you break this cycle?",
    likes: 18,
    comments: 12,
    shares: 2,
  },
...
export default function CommunityPage() {
  ...
  const posts = useMemo(() => {
    const filtered =
      selectedCategory === "All" ? INITIAL_POSTS : INITIAL_POSTS.filter((p) => p.category === selectedCategory)
          ...
          <section className="md:col-span-9 space-y-3 md:space-y-6">
            <div className="space-y-3 md:space-y-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <h1 className="text-sm md:text-base font-medium text-slate-100">Community</h1>
            ...
            {posts.length === 0 ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-8 text-center shadow-sm">
                <p className="text-slate-100">No posts here yet.</p>
                <p className="mt-1 text-sm text-slate-300">Be the first to share your thoughts.</p>
              </div>
            ) : (
